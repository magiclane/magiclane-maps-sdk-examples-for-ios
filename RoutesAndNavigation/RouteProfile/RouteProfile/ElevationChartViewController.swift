...
class ElevationChartViewController: UIViewController, ChartViewDelegate, AxisValueFormatter {
    ...
    func prepareChart() {
        ...
        self.balloonMarker = BalloonMarker(
            color: .darkGray, font: .systemFont(ofSize: 13, weight: .semibold), textColor: .white, insets: .zero)
    ...
    func refreshChartButtons() {
        ...
        if let timeDist = route.getTimeDistance() {
            ...
            if let imgObject = imgDatabase.getImageById(pinIconId) {
                ...
                if let img = imgObject.renderImage(with: imgSize) {
                    ...
                    items.append(
                        RouteElevationButtonItem(type: .startElevation, image: img, value: self.localElevation(Double(startElevation))))

                    let endElevation = profile.getElevation(Int32(timeDist.getTotalDistance()))

                    items.append(
                        RouteElevationButtonItem(type: .endElevation, image: img, value: self.localElevation(Double(endElevation))))
                }
            }
        }
        ...
        if let imgMax = UIImage(systemName: "arrow.down.to.line") {
            ...
            items.append(
                RouteElevationButtonItem(type: .minElevation, image: imgMax, value: self.localElevation(Double(profile.getMinElevation()))))
        }

        if let imgMin = UIImage(systemName: "arrow.up.to.line") {

            items.append(
                RouteElevationButtonItem(type: .maxElevation, image: imgMin, value: self.localElevation(Double(profile.getMaxElevation()))))
        }
    ...
    func refreshChartClimbData() {
        ...
        for section in climbSections {
            ...
            let item = ClimbSectionItem(
                climbSectionObject: section, grade: grade, slope: slope, startDistance: startDistanceString, endDistance: endDistanceString,
                startElevation: startElevationString, endElevation: endElevationString, length: length)
    ...
    func zoomMapBasedOnChart() {
        ...
        if startDist < endDist {
            ...
            mapViewController.center(
                onRoute: route, startDist: startDist, endDist: endDist, rectangle: self.getRouteRectangle(), animationDuration: 0.0)
        }
    }
    ...
    func resetChartData() {
        ...
        guard
            lineChartView.xAxis.axisMinimum < lineChartView.lowestVisibleX
                || lineChartView.xAxis.axisMaximum > lineChartView.highestVisibleX
        else { return }
    ...
    func chartValueSelected(_ chartView: ChartViewBase, entry: ChartDataEntry, highlight: Highlight) {
        ...
        if let coordinates = route.getCoordinateOnRoute(Int32(self.localDistanceToMeters(entry.x))) {
            ...
            highlightSettings.options = Int32(
                HighlightOption.showLandmark.rawValue | HighlightOption.noFading.rawValue | HighlightOption.overlap.rawValue)
    ...
    func stringForValue(_ value: Double, axis: Charts.AxisBase?) -> String {
        ...
        if let entries = axis?.entries, !entries.isEmpty {
    ...
    func areaEdge(margin: CGFloat) -> UIEdgeInsets {
        ...
        let insets = UIEdgeInsets.init(
            top: (mapViewController.view.safeAreaInsets.top + margin) * scale,
            left: margin * scale,
    ...
    func onElevationButtonPressed(_ item: RouteElevationButtonItem) {
        ...
        if let timeDist = route.getTimeDistance() {
            ...
            case .endElevation:
                ...
                lineChartView.highlightValue(
                    x: self.localDistance(Double(timeDist.getTotalDistance())), y: self.localElevation(Double(elevation)), dataSetIndex: 0)
    ...
    func searchClosestEntry(_ y: Double) -> ChartDataEntry? {
        ...
        var foundDataEntry: ChartDataEntry?
