...
class ViewController: UIViewController, MapViewControllerDelegate {
    ...
    func addListButton() {
        ...
        let barButton = UIBarButtonItem.init(
            image: UIImage.init(systemName: "list.star"), style: .done, target: self, action: #selector(showList))
    ...
    func presentLandmarkOnMap(landmark: LandmarkObject, centerLayout: Bool) {
        ...
        let location =
            " " + String(format: "%.6f", landmark.getCoordinates().latitude) + ", "
        ...
        if let image = landmark.getLandmarkImage(CGSize.init(width: 30 * scale, height: 30 * scale)) {
            ...
            attachment.bounds = CGRect(
                x: 0, y: (font1.capHeight - image.size.height).rounded() / 2, width: image.size.width, height: image.size.height)
