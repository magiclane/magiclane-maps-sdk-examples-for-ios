...
class RangeSettingsModel: ObservableObject {
    ...
    func refresh(mode: RangeTransportMode) {
        ...
        if mode == .truck {
            ...
            self.items = [
                ...
                RangeSettingsItem(
                    title: "Avoid Traffic", description: "",
    ...
    func isRangeValueUsed(value: Double, transportMode: RangeTransportMode) -> Bool {
        ...
        if self.presentedRoutes.first(where: { $0.rangeValue == value && $0.transportMode == transportMode }) != nil {
                                ...
                                ScrollViewReader { value in
                                    ...
                                    HStack(alignment: .center, spacing: 10) {
                                        ForEach(Array(model.presentedRoutes.enumerated()), id: \.1.id) { index, item in
                                            ...
                                            RangeValueRoutesItemView(
                                                model: model, item: model.presentedRoutes[index], index: index,
                                                onRangeDeleted: onRangeDeleted, onRangeSelected: onRangeSelected
                                            )
                                            .id(item.id)
                                        }
    ...
    var body: some View {
        ...
        Button {
            ...
            .overlay(
                RoundedRectangle(cornerRadius: 20)
                    .strokeBorder(
                        item.isSelected ? Color(item.routeColor.withAlphaComponent(1.0)) : Color(uiColor: .tertiaryLabel),
