...
class StylesViewController: UITableViewController, ContentStoreObjectDelegate, ContentUpdateDelegate {
    ...
    func refreshWithLocalMaps() {
        ...
        for object in self.mapStyleContext!.getLocalList() {
    ...
    @objc func checkForUpdate(button: UIBarButtonItem) {
        ...
        guard let context = self.mapStyleContext else {
            return
        }
        ...
        context.checkForUpdate { (status: ContentStoreOnlineSupportStatus) in
            ...
            if status == .upToDate {
                ...
                let alert = UIAlertController.init(title: "Info", message: "Map Styles are up to date.", preferredStyle: .alert)
                ...
                let message1 = "New Map Styles available, \nSize: "
    ...
    override func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        if indexPath.section == 0 {
            ...
            contentObject = self.offlineList[indexPath.row]
            
        } else {
            
            contentObject = self.onlineList[indexPath.row]
        }
        
        guard let object = contentObject else {
            return
        }
